...
const OUT_FILE = path.resolve(process.cwd(), 'data', 'news.json')

// NOTE: legacy `news_cache.json` is no longer used. This module now prefers the flat
// `data/news.json` dump for reads and writes so the admin/UI always operate off a single file.
...
async function fetchNewsForGroups(groups, options = {}) {
  ...
  // NOTE: disabled news.json cache - always fetch fresh from sources
  // The API now uses MongoDB exclusively, so we don't need to cache to JSON files anymore
  /*
  try {
    if (options && options.ttlMs !== 0 && fs.existsSync(OUT_FILE)) {
      const dump = JSON.parse(fs.readFileSync(OUT_FILE, 'utf8') || '[]')
      if (Array.isArray(dump) && dump.length > 0) {
        return dump
      }
    }
  } catch (e) {
    console.error('Error reading news.json dump:', e)
  }
  */

  // Always fetch fresh results from sources
  ...
  if ((!articles || articles.length === 0) && !NEWS_API_ONLY) {
    try {
      ...
      articles = rssItems.map(it => {
        let snippet = (it.contentSnippet || '').trim()
        // Remove "The post ... appeared first on..." text
        snippet = snippet.replace(/\n*The post\s+.+?appeared first on\s+.+?\.?\s*$/i, '').trim()
        return {
          title: it.title,
          link: it.link,
          pubDate: it.pubDate,
  ...
  // annotate mentions using variants - prioritize exact matches over partial matches
  for (const art of articles) {
    const text = normalizeText(((art.title || '') + ' ' + (art.content || '') + ' ' + (art.contentSnippet || '')))
    const hits = new Set()
    const matchedVariants = new Set()
    
    // First pass: try exact/word-boundary matches (longer variants first to avoid partial matches)
    const sortedVariants = Array.from(variantToGroups.keys())
    ...
    for (const v of sortedVariants) {
      ...
      const isExactMatch = words.some(w => w === v || w.startsWith(v + ' ') || w.endsWith(' ' + v))
      if (isExactMatch || text.includes(' ' + v + ' ') || text.startsWith(v + ' ') || text.endsWith(' ' + v)) {
        matchedVariants.add(v)
        for (const gid of variantToGroups.get(v)) hits.add(gid)
      }
    }
    
    // Fallback: if no exact matches found, allow substring matching
    if (hits.size === 0) {
      for (const v of sortedVariants) {
        if (!v) continue
        if (text.includes(v)) {
    ...
    art._matchedVariants = Array.from(matchedVariants)
  }
  ...
  // Write a flat dump to data/news.json for direct consumption
  try {
    fs.writeFileSync(OUT_FILE, JSON.stringify(payload, null, 2), 'utf8')
  } catch (e) {
    console.error('write news dump failed', e)
  }
