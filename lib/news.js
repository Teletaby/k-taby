...
async function fetchNewsForGroups(groups, options = {}) {
  ...
  // If a direct dump `data/news.json` exists from a prior refresh and the caller is not forcing a fresh fetch,
  // prefer returning that file's payload so the UI/admin can operate off the dumped file directly.
  try {
  ...
  // No legacy cache read — always fetch fresh results when news.json is not present or when forced.
