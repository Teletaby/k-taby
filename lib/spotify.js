...
async function getAlbum(id) {
  ...
  const tracks = await Promise.all(j.tracks.items.map(async t => {
    ...
    // If track doesn't have artists or has incomplete artist data, fetch full track details
    if (!artists || artists.length === 0 || !artists[0]?.name) {
      try {
        const trackDetails = await fetchSpotify(`tracks/${encodeURIComponent(t.id)}`)
        if (trackDetails && trackDetails.artists) {
          artists = trackDetails.artists
          // Also get preview_url if not available
          if (!preview && trackDetails.preview_url) {
            preview = trackDetails.preview_url
          }
        }
      } catch (e) {
        // If track fetch fails, keep original data
        console.warn(`Failed to fetch track details for ${t.id}:`, e.message)
      }
    }
